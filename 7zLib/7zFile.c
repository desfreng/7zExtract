...
static WRes File_Open(CSzFile *p, const char *name) {
#ifdef USE_WINDOWS_FILE
    ...
    int flags = (0 ? (O_CREAT | O_EXCL | O_WRONLY) : O_RDONLY);
#ifdef O_BINARY
    flags |= O_BINARY;
#endif
...
WRes InFile_Open(CSzFile *p, const char *name) { return File_Open(p, name); }
