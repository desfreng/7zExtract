...
Z7_BRANCH_FUNC_MAIN(ARM64) {
    ...
    BR_EXTERNAL_FOR {
        ...
        for (;;) {
            ...
            v -= 0x90000000;
            if Z7_UNLIKELY((v & 0x9f000000) == 0) {
                UInt32 z, c;
                // v = rotrFixed(v, 8);
                v += flag;
                if Z7_UNLIKELY(v & mask) continue;
                z = (v & 0xffffffe0) | (v >> 26);
        ...
        for (;;) {
            ...
            if Z7_UNLIKELY(
                    ((v - Z7_CONV_BE_TO_NATIVE_CONST32(0x48000001))
                     & Z7_CONV_BE_TO_NATIVE_CONST32(0xfc000003)) == 0)
                break;
        }
        {
        ...
        */
        ...
        for (;;) {
            ...
            */
            ...
            b3 = p[3];
            p += 2;
            if Z7_UNLIKELY((b3 & (b1 ^ 8)) >= 0xf8) break;
            if Z7_UNLIKELY(p >= lim) {ARMT_TAIL_PROC}
            b1 = p[3];
            p += 2;
            if Z7_UNLIKELY((b1 & (b3 ^ 8)) >= 0xf8) break;
        }
        {
